...
class TestBestPracticesAnalyzer:
    ...
    def test_unused_strings_detection(self):
        ...
        warnings = report.get_by_severity('warning')
        assert any('$unused' in s.message and 'never used' in s.message
                  for s in warnings)
...
class TestOptimizationAnalyzer:
    ...
    def test_hex_wildcard_optimization(self):
        ...
        warnings = report.get_by_severity('warning')
        assert any('wildcards' in s.message and 'inefficient' in s.message
                  for s in warnings)
    ...
    def test_cross_rule_duplication(self):
        ...
        assert any('Same plain pattern used in' in s.description
    ...
    def test_complex_condition_warning(self):
        ...
        assert any('deep condition nesting' in s.description
                  for s in report.suggestions)
