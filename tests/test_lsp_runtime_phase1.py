...
def test_runtime_ignores_watched_workspace_symlink_file_changes(
    tmp_path: Path,
) -> None:
    root = tmp_path / "root"
    root.mkdir()
    ...
    runtime = LspRuntime()
    runtime.set_workspace_folders([str(root)])
    runtime.handle_watched_files([FileEvent(uri=path_to_uri(link), type=FileChangeType.Changed)])
...
def test_workspace_folder_setters_reject_invalid_inputs_without_partial_update(
    ...
    with pytest.raises(ValueError, match="Workspace folder paths must not contain null bytes"):
        index.set_workspace_folders(["\x00broken"])
    ...
    with pytest.raises(ValueError, match="Workspace folder paths must not contain null bytes"):
        runtime.set_workspace_folders(["\x00broken"])
