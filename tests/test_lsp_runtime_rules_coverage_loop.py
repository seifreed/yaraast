...
def test_get_rule_link_records_for_document_skips_declaration_records(
    ...
    message = "declaration record for 'solo' must not appear as a document link"
    assert declaration_links == [], message
...
@pytest.mark.skipif(
    _is_root_process(),
    reason="root bypasses file permission checks; docs/test-skips.yml",
)
