...
@pytest.mark.skipif(
    os.name == "nt",
    reason="chmod is not reliable on Windows; docs/test-skips.yml",
)
...
def test_iter_candidate_files_deduplicates_symlink_aliases(tmp_path: Path) -> None:
    ...
    candidates = index.iter_candidate_files()
    assert candidates == [real.resolve()]
...
@pytest.mark.skipif(
    os.name == "nt",
    reason="symlinks to missing targets behave differently on Windows; docs/test-skips.yml",
)
