...
def test_parse_unparenthesized_or_event_statement_preserves_generated_text() -> None:
    parser = EnhancedYaraLParser(
        'rule bool_event { events: $e.metadata.event_type = "LOGIN" or $e.metadata.event_type = "AUTH" condition: $e }'
    )
    ast = parser.parse()
...
def test_parse_event_double_equals_comparison_preserves_generated_text() -> None:
    parser = EnhancedYaraLParser("rule cmp_event { events: $left == $right condition: $left }")
    ast = parser.parse()
