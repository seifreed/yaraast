...
def test_document_highlight_split_yarax_comprehension_declaration_after_for_newline_is_local() -> (
    None
):
    text = """
rule helper { condition: true }
...
def test_document_highlight_split_yarax_dict_value_declaration_after_newline_is_local() -> None:
    text = """
rule value { condition: true }
rule local_ref {
    condition:
...
""".lstrip()
    ...
    highlights = provider.get_highlights(text, _pos(5, 12))
...
def test_document_highlight_split_yarax_second_lambda_parameter_after_newline_is_local() -> None:
    text = """
rule value { condition: true }
rule local_ref {
    condition:
