...
class ASTDiffer:
    ...
    def diff_asts(self, ast1: YaraFile, ast2: YaraFile) -> ASTDiffResult:
        ...
        for rule_name in common_rules:
            if analysis1['rule_signatures'][rule_name] != analysis2['rule_signatures'][rule_name]:
                ...
                result.logical_changes.append(f"Rule '{rule_name}' modified (logic/structure changed)")
        ...
        for condition_key in common_conditions:
            if analysis1['condition_signatures'][condition_key] != analysis2['condition_signatures'][condition_key]:
        ...
        result.has_changes = bool(
            result.logical_changes or
            result.structural_changes or
            result.added_rules or
            result.removed_rules or
            result.modified_rules
        )
    ...
    def _detect_style_changes(self, ast1: YaraFile, ast2: YaraFile, result: ASTDiffResult) -> ASTDiffResult:
        ...
        try:
            ...
            if not result.logical_changes and code1_lines != code2_lines:
                ...
                for line_num, (line1, line2) in enumerate(zip(code1_lines, code2_lines), 1):
                    if line1.strip() == line2.strip() and line1 != line2:
                        result.style_only_changes.append(f"Line {line_num}: whitespace/indentation change")
                    elif line1 != line2:
                        # Check if it's just formatting (same tokens, different spacing)
                        tokens1 = line1.split()
                        tokens2 = line2.split()
                        if tokens1 == tokens2:
                            result.style_only_changes.append(f"Line {line_num}: spacing/formatting change")
...
class AST_Formatter:
    ...
    def format_file(self, input_path: Path, output_path: Optional[Path] = None,
                   style: str = "default") -> Tuple[bool, str]:
        ...
        for operation, results in by_operation.items():
            ...
            if successful:
                ...
                summary[operation] = {
                    ...
                    'avg_rules_per_second': sum(r.rules_count for r in successful) / sum(times) if sum(times) > 0 else 0
                }
