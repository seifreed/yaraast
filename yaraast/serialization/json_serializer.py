...
class JsonSerializer(ASTVisitor[Dict[str, Any]]):
    ...
    def visit_of_expression(self, node) -> Dict[str, Any]:
        return {
            "type": "OfExpression",
            "quantifier": self.visit(node.quantifier) if hasattr(node.quantifier, 'accept') else node.quantifier,
            "string_set": self.visit(node.string_set) if hasattr(node.string_set, 'accept') else node.string_set
        }
